...
class MyAppJarak extends StatefulWidget {
  @override
  _MyAppState createState() => _MyAppState();
}

class _MyAppState extends State<MyAppJarak> {
  ...
  void initState() {
    ...
    geo = Geoflutterfire();
    var center = geo.point(latitude: -3.478810, longitude: 119.144617);
    stream = radius.switchMap((rad) {
      var collectionReference = _firestore.collection('flutter_driver');
                ...
                children: <Widget>[
                  ...
                  MaterialButton(
                    ...
                    onPressed: () {
                      ...
                      var lng = double.parse(_longitudeController.text);
                      _addPoint(lat, lng);
                    },
                    child: Text(
                      'ADD',
...
//      _showHome();
  ...
  void _addPoint(double lat, double lng) {
    var geoFirePoint = geo.point(latitude: lat, longitude: lng);
    _firestore
...
class _StreamTestWidgetState extends State<StreamTestWidget> {
  ...
  void initState() {
    super.initState();
    geo = Geoflutterfire();
    var center = geo.point(latitude: 12.960632, longitude: 77.641603);
    stream = radius.switchMap((rad) {
      var collectionReference = _firestore.collection('flutter_driver');
      return geo.collection(collectionRef: collectionReference).within(
