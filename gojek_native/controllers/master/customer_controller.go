...
func DeleteCustomer(w http.ResponseWriter, r *http.Request) {
	...
	db := configs.Conn()
	delForm, err := db.Exec("DELETE FROM customers WHERE id=?", id)
	defer db.Close()
	if err != nil {
...
func GetCustomerByID(w http.ResponseWriter, r *http.Request) {
	...
	db := configs.Conn()
	selDB, err := db.Query("SELECT  id,name,created_at,updated_at   FROM customers WHERE id=? limit 1", id)
	defer db.Close()
	if err != nil {
		responses.ERROR_mysql(w, http.StatusUnprocessableEntity, err)
		return
...
func GetAllCustomer(w http.ResponseWriter, r *http.Request) {
	...
	db := configs.Conn()
	selDB, err := db.Query("SELECT id,name,created_at,updated_at FROM customers LIMIT  ?, ?", offset, p.Limit)
	if err != nil {
		responses.ERROR_mysql(w, http.StatusUnprocessableEntity, err)
		return
	}
	...
	//defer selDB.Close()
	count, err = models.ModelGetCount("customers")
	if err != nil {
		responses.ERROR_mysql(w, http.StatusUnprocessableEntity, err)
		return
	}
